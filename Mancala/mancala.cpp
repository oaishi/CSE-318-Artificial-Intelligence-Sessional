...
public:
    ...
    enum Algo { GREEDY, ALPHABETA };
    virtual int SelectTurn(int& bestMove, Side side, MancalaBoard board, Algo tactic, int heus , int alpha, int beta) = 0;
...
public:
    ...
    int SelectTurn(int& bestMove, Side side, MancalaBoard board, Algo tactic, int heus, int alpha, int beta);
    bool getTurn() const
    {
        return turn;
    }
...
int Human::SelectTurn(int& bestMove, Side side, MancalaBoard board, Algo tactic, int heus, int alpha, int beta)
{
    int* a = NULL;
    ...
    if (tactic == GREEDY)
    {
        ...
        for (int i=0; i<board.getPit(); i++)
        {
            ...
            if (board.getField(i) != 0)
            {
                ...
                {
                    if(heus>2)
                        a[humanpits] = movevalue + W3 * SelectTurn(dummy, side, dummyboard, GREEDY,heus, alpha, beta);
                    else
            ...
            for (int i=(board.getPit()+1); i<(2*board.getPit()+1); i++)
            {
                ...
                if (board.getField(i) != 0)
                {
                    ...
                    if (reply == STORE)
                    {
                        ...
                        if(heus>2)
                            a[i] = movevalue + W3 * SelectTurn(dummy, CPU, dummyboard, ALPHABETA,heus,alpha, beta);
                        else
                          //  a[humanpits] = movevalue ;
                            a[i] = movevalue + SelectTurn(dummy, CPU, dummyboard, ALPHABETA,heus,alpha, beta); // recursive call to chooseMove
                    ...
                    {
                        if (depth <= max_depth){
                            int tempScore = SelectTurn(dummy, HUMAN, dummyboard, ALPHABETA,heus,alpha, beta); // recursive call to chooseMove
                            a[i] = movevalue - tempScore;
                            int max = INT_MIN;
                            for (int j=0; j<i; j++)
                            {
                                if (a[j] > max)
                            ...
                            if(max<=alpha){
                                ///
                                /*a[humanpits] = movevalue - tempScore;
                                max = a[humanpits];
                                // cout<<i<<" ";*/
                                bestMove = i;
                                return max;
                            }
                            beta = (max<beta)?max:beta;
                        }
                        else if (reply == EMPTY && heus>3)
                        {
            ...
            for (int i=0; i<board.getPit(); i++)
            {
                ...
                if (board.getField(i) != 0)
                {
                    ...
                    if (reply == STORE)
                    {
                        ...
                        step++;
                        if(heus>2)
                            a[humanpits] = movevalue + W3 * SelectTurn(dummy, side, dummyboard, ALPHABETA,heus,alpha, beta);
                        else
                          //  a[humanpits] = movevalue ;
                            a[humanpits] = movevalue + SelectTurn(dummy, HUMAN, dummyboard, ALPHABETA,heus,alpha, beta); // recursive call to chooseMove
                        step--;
                        depth++;
                    ...
                    {
                        if (depth <= max_depth){
                            int tempScore = SelectTurn(dummy, CPU, dummyboard, ALPHABETA, heus,alpha, beta); // recursive call to chooseMove
                            a[i] = movevalue - tempScore;
                            int max = INT_MIN;
                            for (int j=0; j<i; j++)
                            {
...
int Computer::SelectTurn(int& bestMove, Side side, MancalaBoard board, Algo tactic, int heus , int alpha, int beta)
{
    depth++;
    int* a = NULL;
    a = new int[board.getPit()];
    ...
    if (tactic == GREEDY)
    {
        ...
        for (int i=board.getPit()+1; i<(2*board.getPit()+1); i++)
        {
            ...
            if (board.getField(i) != 0)
            {
                ...
                {
                    if(heus>2)
                            a[computerpits] = movevalue + W3 * SelectTurn(dummy, side, boardcopy, GREEDY,heus,alpha, beta);
                        else
            ...
            for (int i=0; i<board.getPit(); i++)
            {
                ...
                if (board.getField(i) != 0)
                {
                    ...
                    if (reply == STORE)
                    {
                        ...
                        step++;
                        if(heus>2)
                            a[i] = movevalue + W3 * SelectTurn(dummy, HUMAN, boardcopy, ALPHABETA,heus,alpha, beta);
                        else
                          //  a[humanpits] = movevalue ;
                            a[i] = movevalue + SelectTurn(dummy, HUMAN, boardcopy, ALPHABETA,heus,alpha, beta); // recursive call to chooseMove
                    ...
                    {
                        if (depth <= max_depth){
                            int tempScore = SelectTurn(dummy, CPU, boardcopy, ALPHABETA, heus,alpha, beta); // recursive call to chooseMove
                            a[i] = movevalue - tempScore;
                            int max = INT_MIN;
                            for (int j=0; j<i; j++)
                            {
            ...
            for (int i=(board.getPit()+1); i<(2*board.getPit()+1); i++)
            {
                ...
                if (board.getField(i) != 0)
                {
                    ...
                    if (reply == STORE)
                    {
                        ...
                        step++;
                        if(heus>2)
                            a[computerpits] = movevalue + W3 * SelectTurn(dummy, CPU, boardcopy, ALPHABETA, heus, alpha, beta); // recursive call to chooseMove
                        else
                            a[computerpits] = movevalue + SelectTurn(dummy, CPU, boardcopy, ALPHABETA, heus, alpha, beta) ;
                        step--;
                        depth++;
                    }
                    else
                    {
                        if (depth <= max_depth){
                            int tempScore = SelectTurn(dummy, HUMAN, boardcopy, ALPHABETA, heus,alpha, beta); // recursive call to chooseMove
                            a[i] = movevalue - tempScore;
                            int max = INT_MIN;
                            for (int j=0; j<i; j++)
                            {
